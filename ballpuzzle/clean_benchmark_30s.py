        ...
        try:
            for event in engine.solve(container, inventory, None, options):
                if event['type'] == 'solution':
                    ...
                    # Show progress every 50 solutions for DLX, 1 for DFS
                    progress_interval = 1 if engine_name == 'DFS' else 50
                    if solutions % progress_interval == 0:
                    ...
                    # Suppress tick events to reduce output
                    pass
                    
        except KeyboardInterrupt:
            print(f'  Interrupted after {solutions} solutions')
            wall_time = time.time() - start_wall_time
            results[engine_name] = {
                'solutions': solutions,
